// Headless regression — SPEC §208: the civil half of the mission tree.
//
...
//
// §208 gives every playable chain a three-strand civil band, marked in the
...
// ---------------------------------------------------------------- shape
console.log('== §208 static: every playable chain grew a civil band ==');
...
// ---------------------------------------------------------------- fencing
console.log('== §208 static: the court strand is fenced and the band is additive ==');
for (const c of CHAINS) {
  const key = c.id + '/' + c.tag;
  const civil = civilOf(c.list);
...
// nothing throws: the panel just draws a medallion that never opens. §208
...
console.log('== §208: every civil reward seats keys the engine reads ==');
{
...
// ------------------------------------------------------------ no free lunch
console.log('== §208 live: nothing civil is accomplished at boot ==');
...
console.log('== §208 live: every civil objective pays in a maximal realm ==');
...
console.log('== §208 live: the AI earns the government and the region, never the court ==');
for (const c of CHAINS) {
  const key = c.id + '/' + c.tag;
...
console.log('== §208: the martial share of the trees ==');
{
