// Headless regression — the crown war (SPEC §225): Herod begins the chapter
...
console.log('== the collar thrown off from below (SPEC §225) ==');
{
...
console.log('== the crown war\'s pen belongs to the claimant (SPEC §225) ==');
{
  const { game, ctx } = boot(BOOKMARK_40, 'HER');
  const w = crownWar(game);
