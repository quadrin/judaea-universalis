...
ok(ml.length === 21 && ml[0].status === 'done' && ml.some((m) => m.status === 'current'), 'getMissions statuses: ' + ml.map((m) => m.status).join(','));
...
console.log('== 132 CE bookmark boots and runs ==');
{
  ...
  ok(a4.getMissions().length === 22, 'JUD 132 missions present (incl. the Third House, SPEC §32, the §179 curriculum, the §192/§197 expansion, and the §183/§192/§197 roads not taken)');
