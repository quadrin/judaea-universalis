// Headless regression — SPEC §207: Beta Israel, Lod, Lag BaOmer, and Himyar.
//
...
// ---------------------------------------------------------------------------
console.log('== the §207 cards are dated cards in their own chapters ==');
{
  const WANT = {
...
console.log('== a funded mission cannot take the ambas to the last household ==');
{
  ...
  // promised three centuries could not. §207 makes the completion respect
...
console.log('== 384: the inscriptions change their god ==');
{
  ...
  // answer — no Himyarite court on the board, and the Najran of the martyrs
  // already Christian. If either drifts, two chapters disagree about the
  // same century, and this is the suite that says so.
  const m529 = POLITICAL_MAPS['529ce'];
  ok(!!m529 && !Object.values(m529.owners).includes('HMY'),
    '529 opens with the Jewish kingdom already fallen — no HMY court on the board');
  ok(m529.religions && m529.religions.Najran === 'christianity',
    'and the Najran of the martyrs of 523 is Christian ground');
