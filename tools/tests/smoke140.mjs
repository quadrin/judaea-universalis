// Headless regression — SPEC §213: the works of one's own. §181 made the air
...
//
//   THE SAVE — a pre-§213 save loads with an empty book and every antique
...
console.log('== the roster: whose shops, and what the panel is told ==');
{
  ...
  // A formed crown keeps its founder's shops (SPEC §102/§213).
...
console.log('== the effects fold, and the saves ==');
{
  ...
  // A pre-§213 save has no book at all.
  const w = world('ISR');
  ...
  'a pre-§213 save revives with an empty book on every court');
