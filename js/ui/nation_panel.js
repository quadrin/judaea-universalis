...
export function createNationPanel(el, { DEFINES, onClose, onPeaceClick, onWarClick, onProvinceClick }) {
  ...
  function build() {
    el.innerHTML = `
      ...
      <!-- What we build ourselves (SPEC §209): the other half of the arms
    ...
    el.addEventListener('click', (e) => {
      ...
      // The works of one's own (SPEC §209): one probe to begin a program,
  ...
  // The works of one's own (SPEC §209), directly under How We Muster: the
  ...
  // actually uses (SPEC §209 pins the list), so nothing prints raw.
  const EFFECT_WORDS = {
