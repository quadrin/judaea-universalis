...
export const FLAGS = {
  ...
  // Hamath: the double-headed eagle of the Neo-Hittite cities, cut in relief
  // at Alaca Hoyuk and carried west on their seals long before any European
  // crown thought of it — set inside the ring of the seal it was pressed
  // with. Two loose attempts at the bird alone both read as a moth; the frame
  // is what tells the eye it is looking at a device rather than an insect.
  HMT:
    `<circle cx="12" cy="12" r="10.4" fill="none" stroke="${FO}" stroke-width="3"/>` +
    `<circle cx="12" cy="12" r="10.4" fill="none" stroke="${FG}" stroke-width="1.6"/>` +
    `<path d="M12 10.6c1.5 0 2.4.9 2.4 2.4 0 1-.3 1.9-1 2.7l.8 3.9H9.8l.8-3.9c-.7-.8-1-1.7-1-2.7 0-1.5.9-2.4 2.4-2.4Z" ${SIL}/>` +
    `<path d="M10.2 11.8 4 10.2l.6 2.2-1.4 1 6.4 2.2Z" ${SIL}/>` +
    `<path d="M13.8 11.8l6.2-1.6-.6 2.2 1.4 1-6.4 2.2Z" ${SIL}/>` +
    `<path d="M11 11.2V8.4c0-1-.7-1.6-1.8-1.6-.8 0-1.4.3-1.8 1l1.6.7-2 .7 1.9 2Z" ${SIL}/>` +
    `<path d="M13 11.2V8.4c0-1 .7-1.6 1.8-1.6.8 0 1.4.3 1.8 1l-1.6.7 2 .7-1.9 2Z" ${SIL}/>` +
    `<path d="M11 17.4h2" ${DET}/>`,

  // Qedar: the dromedary of the incense road. The Assyrian annals count
  // Qedarite wealth in camels because that is what Qedar is — the freight of
  // Arabia walking north to the coast in thousands. Long-legged and long in
  // the neck, so the silhouette cannot be mistaken for anything on a farm.
  QDR:
    `<path d="M7.4 12.9c.9-1 2.2-1.5 3.9-1.5h2.8c1.4 0 2.5.4 3.1 1.3l.4.7c.3.5.4 1 .2 1.6l-.4 1.2c-.3.8-.9 1.3-1.8 1.5H9c-.9-.2-1.5-.8-1.8-1.7Z" ${SIL}/>` +
    `<path d="M9.8 11.6c.4-1.5 1.3-2.3 2.7-2.4 1.3-.1 2.3.7 2.8 2.3l-1.6.5c-.3-.8-.6-1.1-1.1-1.1-.5 0-.9.3-1.1 1.1Z" ${SIL}/>` +
    `<path d="M17 13.1c.3-3.3.9-5.8 1.9-7.6l1.7.9c-.8 1.4-1.3 3.6-1.6 6.5Z" ${SIL}/>` +
    `<path d="M18.9 4.4c1.2-.7 2.3-.7 3.3.1-.7.7-1.1 1.6-1.1 2.6l-2.8-.8Z" ${SIL}/>` +
    `<path d="M21.1 4.2c.7-.3 1.4-.3 2 .2l-1.5 1.2Z" ${ACC}/>` +
    `<path d="M9 17.7h1.5v4.5H9Z M11.6 17.7h1.3v4.5h-1.3Z M14.4 17.7h1.3v4.5h-1.3Z M16.8 17.7h1.5v4.5h-1.5Z" ${SIL}/>` +
    `<path d="M7.4 13.4 5.6 14.6c-.6.4-.8 1.1-.5 1.8l1.5-.5Z" ${SIL}/>` +
    `<path d="M9.6 15.6h7" ${DET}/>` +
    `<path d="M20.6 5.6h.7" ${DET}/>`,
  ...
  // Elam: the serpent of Inshushinak, who guards Susa. Elamite kings sat on
  // serpent thrones and put the coil on their seals for three thousand years,
  // which is longer than most of this table existed. Coiled with the head
  // raised off it, so that it reads as a snake and not as a shell.
  ELA:
    `<path d="M9.8 22c-3.9 0-6.4-2.2-6.4-5.7 0-3.2 2.2-5.4 5.4-5.4 2.8 0 4.6 1.7 4.6 4.2 0 2.1-1.4 3.5-3.4 3.5-1.7 0-2.8-1-2.8-2.5 0-1.2.8-2 2-2v1.9c-.1 0-.2.1-.2.2 0 .2.2.4.5.4.5 0 .9-.4.9-1 0-.9-.7-1.5-1.8-1.5-1.5 0-2.5 1.1-2.5 2.8 0 2 1.5 3.3 3.8 3.3 2.7 0 4.5-1.9 4.5-4.7 0-2.1-.9-3.8-2.5-4.9l1-1.9c2.3 1.5 3.6 3.9 3.6 6.8 0 4-2.8 6.5-6.7 6.5Z" ${SIL}/>` +
    `<path d="M12.6 10.6c-.9-2.3-.5-4.4 1.1-6.2 1.4-1.6 3.3-2.4 5.6-2.4v2.3c-1.6 0-2.8.5-3.8 1.6-.9 1-1.1 2.2-.6 3.6Z" ${SIL}/>` +
    `<path d="M19.3 1.4c1.5 0 2.6.5 3.3 1.6-.8 1-1.9 1.5-3.3 1.5Z" ${ACC}/>` +
    `<path d="M22.6 3l1.4-.6-1.4-.7Z" ${ACC}/>` +
    `<path d="M20.2 2.6h.9" ${DET}/>`,

  // Media: the akinakes, the short iron sword with the lobed pommel and the
  // flared chape that the Persepolis reliefs hang at the hip of every Mede in
  // the procession. It is what the Assyrians were buying when they took
  // Median tribute and what the Medes were carrying when they took Nineveh.
  MDA:
    `<path d="M10.4 8.2h3.2l-.5 9.2c-.1 1.9-.5 3.4-1.1 4.6-.6-1.2-1-2.7-1.1-4.6Z" ${SIL}/>` +
    `<path d="M6.6 6.2h10.8v2.2H6.6Z" ${ACC}/>` +
    `<path d="M10.6 3.2h2.8v3h-2.8Z" ${SIL}/>` +
    `<path d="M9.2 2.6c0-1 .6-1.6 1.6-1.6.7 0 1.2.3 1.5.9l-.6 1.2Z" ${ACC}/>` +
    `<path d="M14.8 2.6c0-1-.6-1.6-1.6-1.6-.7 0-1.2.3-1.5.9l.6 1.2Z" ${ACC}/>` +
    `<path d="M10.1 16.4h3.8l-.2 2.2h-3.4Z" ${ACC}/>` +
    `<path d="M11.1 11.2h1.8" ${DET}/>` +
    `<path d="M8.4 7.3h7.2" ${DET}/>`,

  // Persia: the fire altar — the stepped plinth, the stepped crown and the
  // flame between them, which is the device the Persian kings cut above their
  // own tombs and struck on their coins for a thousand years afterwards. The
  // four-winged genius of Pasargadae is the better relief and it read as an
  // insect at every size this chip is ever drawn at.
  PAS:
    `<path d="M4.8 21.2h14.4v1.8H4.8Z" ${ACC}/>` +
    `<path d="M6.4 18.8h11.2v2.4H6.4Z" ${SIL}/>` +
    `<path d="M8.2 16.6h7.6v2.2H8.2Z" ${SIL}/>` +
    `<path d="M10.2 10.2h3.6v6.4h-3.6Z" ${SIL}/>` +
    `<path d="M8.2 8h7.6v2.2H8.2Z" ${SIL}/>` +
    `<path d="M6.4 5.8h11.2v2.2H6.4Z" ${SIL}/>` +
    `<path d="M12 0.8c2.3 2.4 3.4 4.4 3.4 6.1 0 .3 0 .6-.1.9H8.7c-.1-.3-.1-.6-.1-.9 0-1.7 1.1-3.7 3.4-6.1Z" ${ACC}/>` +
    `<path d="M10.8 13h2.4" ${DET}/>`,

  // Tabal: the grapes and the grain of the Ivriz relief, where the king of
  // Tuwana stands before Tarhunzas and the god is holding a vine in one hand
  // and a sheaf in the other. It is the largest rock carving these highland
  // kingdoms left and the only one that is about what the country grows.
  TAB:
    `<path d="M9.6 6.2h1.5v3.4H9.6Z" ${SIL}/>` +
    `<path d="M6.2 5.4c1.6-1.4 3.4-1.9 5.4-1.5-.8 1.9-2.3 2.8-4.4 2.8Z" ${SIL}/>` +
    `<circle cx="10.4" cy="11.4" r="1.9" ${ACC}/>` +
    `<circle cx="7.3" cy="13.9" r="1.9" ${ACC}/>` +
    `<circle cx="13.5" cy="13.9" r="1.9" ${ACC}/>` +
    `<circle cx="10.4" cy="16.4" r="1.9" ${ACC}/>` +
    `<circle cx="8.4" cy="19.4" r="1.8" ${ACC}/>` +
    `<circle cx="12.4" cy="19.4" r="1.8" ${ACC}/>` +
    `<path d="M18.6 21.4V9.8h1.6v11.6Z" ${SIL}/>` +
    `<path d="M19.4 3.6c1.6 1.8 2.4 3.9 2.4 6.2h-4.8c0-2.3.8-4.4 2.4-6.2Z" ${ACC}/>` +
    `<path d="M16.4 8c-.5-1.9-.2-3.6 1-5.1.9 1.5 1.2 3.1.9 4.8Z M22.4 8c.5-1.9.2-3.6-1-5.1-.9 1.5-1.2 3.1-.9 4.8Z" ${ACC}/>` +
    `<path d="M19.4 6.2v3.6" ${DET}/>`,
