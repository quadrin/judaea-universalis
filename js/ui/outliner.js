...
// The pace a column keeps (SPEC §186) — the sim's own function, so the number
...
export function createOutliner(el, {
  ...
  function buildHtml() {
    ...
    for (const a of armies) {
      ...
      // The three arms by their pattern names (SPEC §186), and the face of
    ...
    if (wars.length) {
      ...
      for (const w of wars) {
        ...
        const lead = opp ? flagChip(opp, ctx.DEFINES, 14, true, ctx.game) : icon('flame', 'icon-row');
        html += `
