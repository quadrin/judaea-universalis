...
// its own rule and opens no crisis (SPEC §213) — the lot is cast again, the
...
export function rulerDies(ctx, tag, causeText) {
  ...
  if (govHas(ctx, tag, 'elects')) {
    ...
    // Nobody inherits here, so nobody's claim died with them (SPEC §213). The
...
export function monthlySuccession(ctx) {
  ...
  for (const tag of Object.keys(g.tags)) {
    ...
    try {
      ...
      // council to seat (SPEC §213) — the Lot draws, the assembly convenes,
