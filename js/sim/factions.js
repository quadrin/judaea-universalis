...
// Favor (SPEC §195): the credit an estate extends a crown that keeps it warm.
...
export function monthlyFactions(ctx) {
  ...
  for (const def of defs) {
    ...
    try {
      ...
      // The favor bank fills from the mood (SPEC §195): a devoted party
...
// ---------------------------------------------------------------------------
// The asks (SPEC §195): what a trusted estate can be leaned on FOR.
//
...
export function getFactionsInfo(ctx) {
  ...
  // the same arithmetic the estates mapmode paints (SPEC §167/§195).
  let shares = null;
