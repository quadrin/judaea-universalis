// Judaea Universalis — where the arms come from (SPEC §181, §209). DOM-free.
...
// and the AI's own deal-seeking — and, below, §209's answer to all of it:
...
export function armsSignGate(ctx, client, supplier) {
  ...
  // Who may sell (SPEC §209): the bookmark's exporters, and any court that
...
export function armsInfo(ctx, me, other) {
  try {
    ...
    // A court that built its own works exports like any arsenal (SPEC §209),
...
export function monthlyArms(ctx) {
  ...
  // built their way onto the list (SPEC §209). Read once: it cannot change
...
// The development line, for the ledger and the AI's own books (SPEC §209).
