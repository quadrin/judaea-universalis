// Judaea Universalis — where the arms come from (SPEC §181, §213). DOM-free.
...
// and the AI's own deal-seeking — and, below, §213's answer to all of it:
...
export function armsSignGate(ctx, client, supplier) {
  ...
  // Who may sell (SPEC §213): the bookmark's exporters, and any court that
...
export function armsInfo(ctx, me, other) {
  try {
    ...
    // A court that built its own works exports like any arsenal (SPEC §213),
...
export function monthlyArms(ctx) {
  ...
  // built their way onto the list (SPEC §213). Read once: it cannot change
...
// The development line, for the ledger and the AI's own books (SPEC §213).
